// pages/api/verify.js
// v3: OTA works + NO name validation on update_guest
// - update_guest no longer blocks on name mismatch (reservation existence is enough)
// - still stores the REAL Cloudbeds reservationId in cloudbeds_reservation_id
// - verify_face re-fetches using cloudbeds_reservation_id (preferred) and won't overwrite with null
...
const BUILD_ID = "cloudbeds-integration-v1-ota-no-name-check-v3";
...
function getGuestIsCheckedIn(cloudbeds) {
  ...
  if (typeof v === "string") {
    ...
    if (["true", "yes", "y", "checkedin", "checked_in", "checked-in", "1", "inhouse", "in_house"].includes(s)) return true;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "update_guest") {
      ...
      // authoritative flow type from DB
      ...
      if (!sess) return safeJson(res, 404, { error: "Session not found" });

      const flowType = normalizeFlowType(sess.flow_type);

      // visitor path (no cloudbeds)
      if (flowType === "visitor") {
        const { error: updateErr } = await supabase
          .from("demo_sessions")
          .update({
        ...
        return safeJson(res, 200, { success: true, flow_type: "visitor", guest_name, room_number: "VISITOR" });
      }

      // guest path (cloudbeds)
      try {
        const cloudbeds = await fetchCloudbedsReservation(booking_ref);

        // OPTIONAL gate (keep or remove). If you want ZERO gating, delete this block.
        const checkedIn = getGuestIsCheckedIn(cloudbeds);
        if (!checkedIn) return safeJson(res, 400, { error: "guest check in required" });

        const realRid = cloudbeds?.reservationId || null;

        const { error: updateErr } = await supabase
          .from("demo_sessions")
          .update({
            guest_name,                    // keep what user typed
            room_number: String(booking_ref), // store what they typed (OTA / source id)
            cloudbeds_reservation_id: realRid, // store REAL Cloudbeds reservationID
    ...
    if (action === "upload_document") {
      ...
      // (optional) Textract validation remains; remove if you want zero name checks anywhere
      if (flowType === "guest" && sess.guest_name) {
        try {
          const analyzeRes = await textract.send(
            new AnalyzeIDCommand({ DocumentPages: [{ Bytes: imageBuffer }] })
          );
          ...
          // If you want ZERO name checks anywhere, delete the next block.
          if (docFullName && String(sess.guest_name).trim() && docFullName.toLowerCase() !== String(sess.guest_name).trim().toLowerCase()) {
            // soft fail (don’t block): comment this return if you want to allow any ID
            // return safeJsonF
          }
        } catch {
          // ignore
